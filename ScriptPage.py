...
class ScriptPage(QMainWindow, Ui_MainWindow):
    def __init__(self,lineEdit_Scripts,Button_ScriptsBrowse,textEdit_Scripts,Button_scriptConfirm,
                 lineEdit_SaveResult,Button_SaveResultBrowse,Button_scriptUpdate):
        super().__init__()
        ...
        Button_scriptUpdate.clicked.disconnect() if Button_scriptUpdate.receivers(Button_scriptUpdate.clicked) > 0 else None
        Button_scriptUpdate.clicked.connect(self.update_script)
    ...
    def browse_folder_Savescript(self):
        ...
        if folder_path:
            ...
            # 将处理后的路径显示在文本框中
            self.lineEdit_SaveResult.setText(normalized_path)
    ...
    def update_script(self):
        ...
        # 检查路径是否有效
        if os.path.isfile(file_path):
            # 将 textEdit_Scripts 中的内容保存回文件
            with open(file_path, 'w', encoding='utf-8') as file:
                file.write(self.textEdit_Scripts.toPlainText())
        else:
            print("文件路径无效，无法保存。")